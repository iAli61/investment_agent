...
- **Port conflicts**: If ports 8000 or 3000 are already in use, specify alternative ports:
  ```bash
...
2. Run the container:
```bash
docker run -p 3000:3000 -p 8000:8000 -e DATABASE_URL=sqlite:///./investment_analysis.db -e OPENAI_API_KEY=your_openai_api_key investment-agent
```
...
2. **Azure Container Apps**: For hosting the React frontend
...
### User Interface
- React-based web interface with interactive components
- Responsive design with clear data visualization
- Interactive forms for property data input
- Real-time calculation updates
