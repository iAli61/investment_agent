...
const MainLayout = () => {
  ...
  const handleResizeMove = (e) => {
    ...
    // Set min and max limits for the sidebar width
  ...
  // Attach event listeners when isResizing changes
  useEffect(() => {
    if (isResizing) {
      document.addEventListener('mousemove', handleResizeMove);
      document.addEventListener('mouseup', handleResizeEnd);
  ...
  const ChatSidebar = () => (
    <Box 
      sx={{ 
        width: { xs: '100%', md: chatSidebarWidth },
        minWidth: { md: 250 },
        maxWidth: { md: 500 },
  ...
  const Resizer = () => (
    <Box
      ...
      sx={{
        ...
        opacity: 1,
        position: 'relative',
        userSelect: 'none', // Prevent text selection on the resizer itself
      }}
    ...
    >
      <Box 
        sx={{ 
          ...
          height: 50,
          width: 20,
          backgroundColor: isResizing ? 'primary.main' : 'background.paper',
          borderRadius: 1,
          boxShadow: isResizing ? 2 : 0,
  ...
  return (
    ...
    >
      ...
      {/* Flex container for Chat Sidebar and Resizer */}
      <Box sx={{ 
        display: 'flex', 
        height: '100%',
        ...
        {(chatSidebarOpen || !isMobile) && !isMobile && <Resizer />}
      </Box>
      ...
      <Box 
        sx={{ 
          ...
          width: { 
            xs: '100%',
            md: chatSidebarOpen ? `calc(100% - ${chatSidebarWidth}px - 5px)` : '100%'
          },
          transition: theme => theme.transitions.create(['width', 'margin'], {
            easing: theme.transitions.easing.easeOut,
            duration: isResizing ? 0 : theme.transitions.duration.standard,
          }),
        }}
      >
        {/* Top AppBar */}
        ...
        {/* Main Content - where page content will be rendered */}
        <Box 
          sx={{ 
            display: 'grid',
